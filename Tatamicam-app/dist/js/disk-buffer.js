...
export class DiskBuffer {
    ...
    pendingBlobs = [];
    pendingStartTime = 0;
    currentTime = 0; // tempo acumulado em segundos
    segmentDuration = 6; // segundos por arquivo
    constructor(maxSeconds = 300, mimeType = 'video/webm') {
    ...
    async _flushPendingSegment() {
        if (this.pendingBlobs.length === 0 || !this.tempDir)
            return;
    ...
    async _trim() {
        if (!this.tempDir || this.maxSeconds === Infinity)
            return;
    ...
    async getSegmentsBetween(startSec, endSec) {
        if (!this.tempDir || this.segments.length === 0)
            return [];
        const tauri = window.__TAURI__;
        const result = [];
