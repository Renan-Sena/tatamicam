...
export async function getHwid() {
    ...
    if (!isTauri())
        return null;
    try {
        cachedHwid = await window.__TAURI__.core.invoke('get_hwid');
        return cachedHwid;
    }
    catch (e) {
...
export async function activateLicense(tokenCode, force = false) {
    const hwid = await getHwid();
    if (!hwid)
        return { success: false, message: 'Não foi possível obter identificação do hardware.' };
    try {
...
export async function verifyLicense() {
    ...
    if (!jwt || !tokenCode)
        return { valid: false, blocked: true, reason: 'Nenhuma licença encontrada. Ative o produto.' };
    const hwid = await getHwid();
    if (!hwid)
        return { valid: false, blocked: true, reason: 'Não foi possível confirmar o hardware.' };
    const result = await window.__TAURI__.core.invoke('verify_license_jwt', { jwt, expectedHwid: hwid });
    if (!result.valid) {
        try {
