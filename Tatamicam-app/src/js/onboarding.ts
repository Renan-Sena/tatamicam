...
const btnObBack = document.getElementById('btnObBack') as HTMLButtonElement;
const btnObNext = document.getElementById('btnObNext') as HTMLButtonElement;
const btnObSkip = document.getElementById('btnObSkip') as HTMLButtonElement;
const btnGrant = document.getElementById('btnGrantPerm') as HTMLButtonElement | null;
const permIcon = document.getElementById('permIcon') as HTMLElement | null;
const permDesc = document.getElementById('permDesc') as HTMLElement | null;
const permStat = document.getElementById('permStatus') as HTMLElement | null;
...
export function initOnboarding(): void {
    ...
    obDots.forEach(d => {
        d.addEventListener('click', () => goStep(parseInt((d as HTMLElement).dataset.i!)));
    });
...
async function onGrant(): Promise<void> {
    ...
    if (ok) {
        await populateCameraList(document.getElementById('camSel') as HTMLSelectElement);
        syncPermUI();
    } else {
        if (permStat) {
...
function goStep(n: number): void {
    ...
    if (currentStep === 1) syncPermUI();
    if (currentStep !== 1) btnObNext.disabled = false;
}

function syncPermUI(): void {
    ...
    if (permGranted) {
        ...
        if (permDesc) permDesc.textContent = 'Câmeras disponíveis! Você pode prosseguir.';
        if (btnGrant) btnGrant.disabled = true;
        if (permStat) {
        ...
        if (permDesc) permDesc.textContent = 'Clique no botão abaixo e permita o acesso à câmera quando o navegador perguntar.';
        if (btnGrant) btnGrant.disabled = false;
        if (permStat) permStat.innerHTML = '';
        btnObNext.disabled = false;
    }
}
