...
export function initActivateModal(): void {
    ...
    document.getElementById('activateModalOverlay')?.addEventListener('click', (e) => {
        if (e.target === document.getElementById('activateModalOverlay')) closeActivateModal();
    });
    document.getElementById('btnActivateToken')?.addEventListener('click', handleActivate);
}

export function openActivateModal(): void {
    ...
    if (overlay) {
        ...
        if (existingTransferBtn) existingTransferBtn.remove();
    }
}
...
function showTransferButton(): void {
