...
interface DvrFeed {
    ...
    url: string;
    stopped: boolean;
    windowStart: number; // início da janela carregada (segundos)
    windowEnd: number;   // fim da janela carregada (segundos)
}
...
function pushChunk(slot: CameraSlot, blob: Blob): void {
...
async function feedDvrIfActive(slot: CameraSlot, blob: Blob): Promise<void> {
    if (slot.bufferMode !== 'ram' && slot.bufferMode !== 'disk' && slot.bufferMode !== 'disk-full') return;

    if (!slot.dvrFeed || slot.dvrFeed.stopped) return;
 ...
 */
export function bufSecSmooth(slot: CameraSlot): number {
    if (!slot.bufSecGrowing || !slot.lastChunkAt) return slot.bufSec;
...
/* ─── Câmera ─── */
async function openCamera(slot: CameraSlot, res: string | undefined, fps: string | undefined): Promise<MediaStream> {
    if (!state.permGranted) {
        const ok = await requestPermission();
        if (!ok) throw new Error('Permissão de câmera negada.');
    }
    let wC: { ideal: number }, hC: { ideal: number };
    if (!res || res === 'original') { wC = { ideal: 1920 }; hC = { ideal: 1080 }; }
    else { const [w, h] = res.split('x').map(Number); wC = { ideal: w }; hC = { ideal: h }; }
    const fpsC = !fps || fps === 'auto' ? { ideal: 30 } : { ideal: parseInt(fps, 10) };
    const vidCon: MediaTrackConstraints = { width: wC, height: hC, frameRate: fpsC };
    if (slot.deviceId) vidCon.deviceId = { exact: slot.deviceId };
    try { const s = await navigator.mediaDevices.getUserMedia({ video: vidCon, audio: false }); applySettings(slot, s, res, fps); return s; } catch (_) { }
    try { const s = await navigator.mediaDevices.getUserMedia({ video: slot.deviceId ? { deviceId: { exact: slot.deviceId } } : true, audio: false }); applySettings(slot, s, res, fps); return s; } catch (_) { }
    try { const s = await navigator.mediaDevices.getUserMedia({ video: true, audio: false }); applySettings(slot, s, res, fps); return s; } catch (e3) { throw new Error('Câmera inacessível: ' + (e3 instanceof Error ? e3.message : String(e3))); }
}

function applySettings(slot: CameraSlot, s: MediaStream, res: string | undefined, fps: string | undefined): void {
...
/* ─── API pública ─── */
export function getState(): GlobalState { return state; }
export function getActiveSlot(): CameraSlot { return state.slots[state.activeSlot]; }
export function setActiveSlot(index: number): void { if (index >= 0 && index < 4) state.activeSlot = index; }
export function setPerSlotDelay(enabled: boolean): void { state.perSlotDelay = enabled; }

export function setMaxBufSec(val: number, slotIndex?: number): void {
    const slot = slotIndex !== undefined ? state.slots[slotIndex] : getActiveSlot();
    slot.maxBufSec = val;
    if (slot.bufferMode === 'ram') trimBuffer(slot);
    if (slot.diskBuffer && (slot.bufferMode === 'disk' || slot.bufferMode === 'disk-full')) slot.diskBuffer.maxSeconds = val;
}

export function setBufferMode(mode: string, slotIndex?: number): void {
    const slot = slotIndex !== undefined ? state.slots[slotIndex] : getActiveSlot();
    if (slot.cameraOn) { showToast('Altere o modo de buffer antes de iniciar a câmera.', true); return; }
    clearBuffer(slot);
    slot.bufferMode = mode as BufferMode;
}

export async function requestPermission(): Promise<boolean> {
    if (state.permGranted) return true;
    if (_permPromise) return _permPromise;
    _permPromise = (async () => {
        try { const tmp = await navigator.mediaDevices.getUserMedia({ video: true, audio: false }); tmp.getTracks().forEach(t => t.stop()); state.permGranted = true; return true; }
...
export async function populateCameraList(selectEl: HTMLSelectElement): Promise<void> {
    try {
        const devs = await navigator.mediaDevices.enumerateDevices();
        const vids = devs.filter(d => d.kind === 'videoinput');
        if (!vids.length) { selectEl.innerHTML = '<option value="">Nenhuma câmera detectada</option>'; return; }
        ...
        vids.forEach((d, i) => {
            ...
            if ((curId && d.deviceId === curId) || (!curId && d.deviceId === prev)) o.selected = true;
            selectEl.appendChild(o);
        });
    } catch (_) { }
}

export async function startCamera(deviceId: string | undefined, res: string | undefined, fps: string | undefined, liveVid: HTMLVideoElement, slotIndex?: number): Promise<MediaStream> {
    const slot = slotIndex !== undefined ? state.slots[slotIndex] : getActiveSlot();
...
export function stopCamera(slotIndex?: number): void {
    const slot = slotIndex !== undefined ? state.slots[slotIndex] : getActiveSlot();
    if (slot.stream) { slot.stream.getTracks().forEach(t => t.stop()); slot.stream = null; }
    slot.cameraOn = false;
}

function calculateBitrate(w: number, h: number, fps: number): number {
    const pixels = w * h;
...
export async function startRecorder(slotIndex?: number): Promise<void> {
    const slot = slotIndex !== undefined ? state.slots[slotIndex] : getActiveSlot();
    if (!slot.stream) return;
    if (slot.recorder && slot.recorder.state !== 'inactive') slot.recorder.stop();
    ...
    const onData = async (e: BlobEvent): Promise<void> => {
        ...
        if (slot.bufferMode === 'disk' || slot.bufferMode === 'disk-full') {
            if (slot.diskBuffer) await slot.diskBuffer.addChunk(e.data);
    ...
    for (const m of preferredMimes) {
        ...
        try {
            ...
            if (!rec) throw new Error('MediaRecorder não foi criado');
            const recorder = rec;
...
export async function stopRecorder(slotIndex?: number): Promise<void> {
    const slot = slotIndex !== undefined ? state.slots[slotIndex] : getActiveSlot();
...
export async function clearBuffer(slot?: CameraSlot): Promise<void> {
...
function resolveReplayCodec(slot: CameraSlot): string {
    const candidates = [
        slot.recordMime,
        'video/webm; codecs="vp8"',
        'video/webm; codecs="vp9"',
...
export async function prepareReplaySource(slot: CameraSlot, seekSec?: number): Promise<{ url: string; ready: Promise<void> }> {
    ...
    const ready = new Promise<void>((resolve, reject) => {
        mediaSource.addEventListener('sourceopen', async () => {
            try {
                ...
                if (!MediaSource.isTypeSupported(codec)) throw new Error(`Codec de replay não suportado: ${codec}`);
                ...
                if (slot.bufferMode === 'disk' || slot.bufferMode === 'disk-full') {
                    if (!slot.diskBuffer) throw new Error('Buffer de disco não inicializado');
...
async function appendBufferSafely(sb: SourceBuffer, data: Uint8Array): Promise<void> {
 ...
 */
export async function prepareReplayBlobUrl(slot: CameraSlot): Promise<string> {
    if (slot.replayUrl) { URL.revokeObjectURL(slot.replayUrl); slot.replayUrl = null; }
    let url: string | null = null;
    if (slot.bufferMode === 'disk' || slot.bufferMode === 'disk-full') {
        if (!slot.diskBuffer) throw new Error('Buffer de disco não inicializado');
        url = await slot.diskBuffer.getUrl();
    } else {
        if (!slot.chunks.length) throw new Error('Nenhum dado no buffer');
        const blob = new Blob(slot.chunks, { type: slot.recordMime || 'video/webm' });
        url = URL.createObjectURL(blob);
    }
    if (!url) throw new Error('Nenhum dado gravado para replay');
    slot.replayUrl = url;
    return url;
}

export function stopDvrFeed(slot: CameraSlot): void {
    if (slot.dvrFeed) {
...
export async function restartCapture(deviceId: string, res: string, fps: string, liveVid: HTMLVideoElement, slotIndex?: number): Promise<void> {
    const slot = slotIndex !== undefined ? state.slots[slotIndex] : getActiveSlot();
    if (!slot.stream || state.restarting) return;
    ...
    } catch (e) { showToast(e instanceof Error ? e.message : 'Erro ao reiniciar captura', true); }
    finally { state.restarting = false; }
}

export async function loadMoreSegments(slot: CameraSlot, direction: 'forward' | 'backward'): Promise<void> {
    if (!slot.dvrFeed || slot.dvrFeed.stopped) return;
    if (slot.bufferMode !== 'disk' && slot.bufferMode !== 'disk-full') return;
    if (!slot.diskBuffer) return;
    ...
    if (newStart === feed.windowStart && newEnd === feed.windowEnd) return;
...
export class FrameDelayer {
    ...
    frameBuffer: { url: string; timestamp: number }[];
    targetFPS: number;
    ...
    _loop(timestamp: number): void { 
        ...
        if (video.readyState >= 2 && video.videoWidth > 0) { 
            ...
            if (timestamp - this.lastCaptureTime >= captureInterval) { this.lastCaptureTime = timestamp; this._captureFrame(timestamp); } const threshold = timestamp - this.delaySeconds * 1000; while (this.frameBuffer.length > 1 && this.frameBuffer[0].timestamp < threshold) { const old = this.frameBuffer.shift(); if (old) URL.revokeObjectURL(old.url); } if (this.frameBuffer.length > 0) { const oldest = this.frameBuffer[0]; const img = new Image(); img.onload = () => { if (this.canvas.width !== img.width || this.canvas.height !== img.height) { this.canvas.width = img.width; this.canvas.height = img.height; } this.ctx.drawImage(img, 0, 0); }; img.src = oldest.url; } } this.animationId = requestAnimationFrame(this._loop.bind(this)); }
    _captureFrame(timestamp: number): void { const w = this.video.videoWidth; const h = this.video.videoHeight; if (this.offscreen.width !== w || this.offscreen.height !== h) { this.offscreen.width = w; this.offscreen.height = h; this.offscreenCtx = this.offscreen.getContext('2d')!; } this.offscreenCtx.drawImage(this.video, 0, 0, w, h); this.offscreen.toBlob(blob => { if (blob) { const url = URL.createObjectURL(blob); this.frameBuffer.push({ url, timestamp }); } }, 'image/jpeg', this.jpegQuality); }
}
